...
(defmacro try-param
  [param-index type-label param-expr]
  `(try ~param-expr
     (catch Exception e#
       (throw (IllegalArgumentException.
...
(defmacro each-param-expr
  ...
  `(~setter ~pstmt-sym ~param-index
     (try-param ~param-index ~type-label ~param-expr)))
...
(defn lay-param-expr
  ...
  (case param-type
    ...
    :date       `(let [p# (try-param ~param-index "date" ~param-value-sym)]
                   (cond
                     (instance? Date p#) (.setDate ~pstmt-sym ~param-index ^java.sql.Date p#)
                     ;; calendar
                     (instance? Calendar p#) (.setDate ~pstmt-sym ~param-index
                                               (Date. (.getTimeInMillis ^java.util.Calendar p#))
                                               ^java.util.Calendar p#)
                     (vector? p#) (let [[^java.sql.Date dt# arg#] p#]
                                    (cond
                                      (string? arg#)   (.setDate ~pstmt-sym ~param-index dt# (tz-cal arg#))
                                      (named? arg#)    (.setDate ~pstmt-sym ~param-index dt# (tz-cal (as-str arg#)))
                                      (instance?
                                        Calendar arg#) (.setDate ~pstmt-sym ~param-index dt# arg#)
                                      :otherwise       (expected "keyword/string timezone or java.util.Calendar" arg#)))
                     (nil? p#)    (.setDate ~pstmt-sym ~param-index nil)
                     :otherwise   (expected "java.sql.Date or java.util.Calendar instance" p#)))
    ...
    :time       `(let [p# (try-param ~param-index "time" ~param-value-sym)]
                   (cond
                     (instance? Date p#) (.setTime ~pstmt-sym ~param-index ^java.sql.Time p#)
                     ;; calendar
                     (instance? Calendar p#) (.setTime ~pstmt-sym ~param-index
                                               (Time. (.getTimeInMillis ^java.util.Calendar p#))
                                               ^java.util.Calendar p#)
                     (vector? p#) (let [[^java.sql.Time time# arg#] p#]
                                    (cond
                                      (string? arg#)   (.setTime ~pstmt-sym ~param-index time# (tz-cal arg#))
                                      (named? arg#)    (.setTime ~pstmt-sym ~param-index time# (tz-cal (as-str arg#)))
                                      (instance?
                                        Calendar arg#) (.setTime ~pstmt-sym ~param-index time# arg#)
                                      :otherwise       (expected "keyword/string timezone or java.util.Calendar" arg#)))
                     (nil? p#)    (.setTime ~pstmt-sym ~param-index nil)
                     :otherwise   (expected "java.sql.Time or java.util.Calendar instance" p#)))
    :timestamp  `(let [p# (try-param ~param-index "timestamp" ~param-value-sym)]
                   (cond
                     (instance? Timestamp p#) (.setTimestamp ~pstmt-sym ~param-index ^java.sql.Timestamp p#)
                     ;; calendar
                     (instance? Calendar p#) (.setTimestamp ~pstmt-sym ~param-index
                                               (Timestamp. (.getTimeInMillis ^java.util.Calendar p#))
                                               ^java.util.Calendar p#)
                     (vector? p#) (let [[^java.sql.Timestamp ts# arg#] p#]
                                    (cond
                                      (string? arg#)   (.setTimestamp ~pstmt-sym ~param-index ts# (tz-cal arg#))
                                      (named? arg#)    (.setTimestamp ~pstmt-sym ~param-index ts# (tz-cal (as-str arg#)))
                                      (instance?
                                        Calendar arg#) (.setTimestamp ~pstmt-sym ~param-index ts# arg#)
                                      :otherwise       (expected "keyword/string timezone or java.util.Calendar" arg#)))
                     (nil? p#)    (.setTimestamp ~pstmt-sym ~param-index nil)
                     :otherwise   (expected "java.sql.Timestamp or java.util.Calendar instance" p#)))
    (expected supported-sql-types param-type)))
